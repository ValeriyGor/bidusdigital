  ...
  function(e) {
    if ($(window).width() > '1024' && $(this).hasClass('arrow-down')){
      if(!$('.curtain').is(":visible")){
        $(".curtain").stop(true,true).fadeIn(100);
      }else{
        $(".curtain").stop(true,true).fadeIn(100);
      }
      ...
      if($('.head-menu__dropdown').is(":visible")){
        ...
        $(this).children(".head-menu__dropdown").stop().fadeTo(100, 1);
      }
    ...
    if ($(window).width() > '1024'){               
      ...
      if(!$('li.arrow-down').is(e.target)){
        $(this).children(".head-menu__dropdown").fadeOut(100);
      }        
    }
  }
);
...
$( ".curtain" ).click(function(e) {
  $(".fast-link").slideUp(0);
  $(".curtain").removeClass('search-active');
...
$( ".worker-blog__item" ).click(function(e) {
  slideWorkerIndex = $(this).index();
...
function putDataWorkerModal(objWorker){  
  $('.modal_worker-blog__head').text(objWorker.attr('data-author'));
  $('.modal_worker-blog__location').text(objWorker.attr('data-location'));
  $('.modal_worker-blog__content').html(objWorker.attr('data-content'));
  $('.modal_worker-blog__image').attr('style', 'background: url("' + objWorker.attr('data-big_img') + '") center no-repeat');
}
...
$( ".sent-gift" ).click(function(e) {
  ...
  if($(window).width() < '768'){
    $('body,html').animate({scrollTop: $(this).parents('.step-one').offset().top -70}, 300);
  }
});
$( ".wait-gift").click(function(e) {
    //$('.check-data').fadeOut(0);
    $('.success-send').not('.small-success').fadeIn(300);
    $('.step-one').addClass('smalling');
    if($(window).width() < '768'){
      $('body,html').animate({scrollTop: $(this).parents('.step-one').offset().top -65}, 300);
    }
  });
...
$( ".close-input" ).click(function(e) {
  ...
  if ($(window).width() > '767'){
	...
	placeholder = $(".search input").attr('data-placeholder');
...
$('.print-version').click(function(e) {
...
$(".show-all").click(function(e) {
  ...
  if ($(window).width() < '767'){
    $(this).parents('.slick-list').css('height', $(this).parents('.slick-slide').height() + 25 + 'px');
  }    
});
...
$( ".search input" ).focus(function() {     
  ...
  $(".curtain").fadeIn(300);
  $(".curtain").addClass('search-active');
...
$('.hidden-message').click(function(){
  ...
  $(this).parents('.form-call').addClass('small-visible');
    ...
    if($(window).width() < '768'){
      $('body,html').animate({scrollTop: $(this).parents('.row').offset().top -100}, 300);
    }
});
 ...
 $('.subjects').on('change', function() {
  let k = $('.subjects option:selected').text();
  $('.trigger').text(k);
});
...
$(document).ready(function(){
  ...
  var tooltip = $(".tooltip-block")
	...
	$('body').delegate('.go_to', 'click', function(){ // ловим клик по ссылке с классом go_to
...
$(document).ready(function() {
  ...
  function func() {  
	// Animate first slide on first page
    // anim();
  }
...
// setWidthScrollblock();
	...
	wnd.scroll(function(){  
		...
		if($(window).width() < '767' && $('.head-menu:visible').length){ mobileMenu = true; } else { mobileMenu = false; }
		
		if(wnd.scrollTop() > headerHeight && wnd.scrollTop() > lastTop && !$('header .search.active').length && !$('.head-menu__dropdown:visible').length && !mobileMenu){
		...
		if ($(window).width() > '1024'){    
			...
			if ($("#techno-carousel").length != 0) { 
				var scrollTop =  $('#techno-carousel').offset().top - $(window).height() + 200;
				if($(this).scrollTop() > scrollTop){
					var percent = $(window).height()/100;
					var res = ($(this).scrollTop() - scrollTop)/percent;
					var wdth = $('#techno-carousel').width();
					var percWidth = ($(".technology > .row").width() + ($(window).width()-$('.technology > .row').width())/2)/wdth * 100;
